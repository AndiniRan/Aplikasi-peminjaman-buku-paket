...
<div class="member-page" data-member-type="guru">
    <div class="member-card">
        <div class="member-title">
            <h2>Daftar seluruh guru</h2>
        </div>

        <div class="member-main-actions">
            <a href="{{ route('admin.member.guru.create') }}" class="member-btn member-btn-add">
                <i class="bi bi-plus-lg"></i>
                <span>Tambah Guru</span>
            </a>

            <form action="{{ route('admin.member.guru.import') }}" method="POST" enctype="multipart/form-data" id="importGuruForm" class="member-import-form">
                @csrf
                <input type="file" name="file_excel" id="importGuruFile" accept=".xlsx,.xls,.csv" hidden>
                <button type="button" class="member-btn member-btn-import" id="importGuruButton">
            ...
            <table class="member-table">
                ...
                <tbody>
                    @forelse($guru as $item)
                        <tr class="member-row"
                            data-search="{{ strtolower($item->email . ' ' . $item->name . ' ' . $item->status) }}">
                            ...
                            <td>
                                <div class="member-actions">
                                    <button
                                        ...
                                        data-edit="{{ route('admin.member.guru.edit', $item) }}"
                                        data-form="deleteGuru{{ $item->id }}">
                                        <i class="bi bi-eye-fill"></i>
                                    </button>

                                    <a href="{{ route('admin.member.guru.edit', $item) }}" class="member-action-btn edit" title="Edit">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>

                                    <button type="button" class="member-action-btn delete member-delete" data-form="deleteGuru{{ $item->id }}" title="Hapus">
                                        <i class="bi bi-trash-fill"></i>
                                    </button>
                                </div>

                                <form action="{{ route('admin.member.guru.destroy', $item) }}" method="POST" id="deleteGuru{{ $item->id }}" class="member-delete-form">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            </td>
                        ...
                        <tr id="memberDatabaseEmpty">
                            <td colspan="6">
                                <div class="member-empty">
                                    ...
                                    <span>Silakan tambahkan guru terlebih dahulu.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                    ...
                    @if($guru->count() > 0)
                        <tr id="memberSearchEmpty">
                            <td colspan="6">
                                <div class="member-empty">
                                    <i class="bi bi-search"></i>
                                    <strong>Guru tidak ditemukan</strong>
        ...
        <div class="member-detail-body">
            ...
            <div class="member-detail-info">
                ...
                <div class="member-detail-row">
                    ...
                    <span class="detail-separator">:</span>
                    <span class="member-status aktif" id="detailStatus"></span>
                </div>
            </div>
        </div>
...
<div class="member-delete-modal" id="memberDeleteModal">
    <div class="member-delete-box">
        ...
        <h3>Hapus Guru?</h3>
        <p>Data guru yang sudah dihapus tidak dapat dikembalikan.</p>

        <div class="member-delete-actions">
            <button type="button" class="modal-cancel" id="memberCancelDelete">
                Batal
            </button>

            <button type="button" class="modal-delete" id="memberConfirmDelete">
                Hapus
            </button>
        </div>
    </div>
