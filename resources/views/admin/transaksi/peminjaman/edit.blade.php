...
<div class="transaksi-form-page" data-form="peminjaman">
    <div class="transaksi-form-card">
        <div class="transaksi-form-title">
            ...
            <p>Silakan ubah data peminjaman buku pada form di bawah ini</p>
        </div>
        ...
        <form action="{{ route('admin.transaksi.peminjaman.update', $transaksi) }}" method="POST" class="transaksi-form">
            @csrf
            @method('PUT')
                    ...
                    <div class="transaksi-custom-select-menu">
                        @foreach($member as $item)
                            ...
                            <button type="button"
                                    ...
                                    {{ old('user_id', $transaksi->user_id) == $item->id ? 'active' : '' }}"
                    ...
                    <select name="user_id" id="user_id" class="transaksi-hidden-select" required>
                        @foreach($member as $item)
                            <option value="{{ $item->id }}"
                                    @selected(old('user_id', $transaksi->user_id) == $item->id)>
                                {{ $item->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="transaksi-custom-select" data-form-select data-target="buku_id">
                    ...
                    <div class="transaksi-custom-select-menu">
                        @foreach($buku as $item)
                            @php
                                $stokUntukEdit = $item->stok_tersedia;

                                if ($item->id === $transaksi->buku_id) {
                                    $stokUntukEdit++;
                                }
                            @endphp
                            ...
                            <button type="button"
                                    class="transaksi-custom-select-option
                                    {{ old('buku_id', $transaksi->buku_id) == $item->id ? 'active' : '' }}"
                                    data-form-select-option
                                    data-value="{{ $item->id }}"
                                    data-stock="{{ $stokUntukEdit }}">
                                {{ $item->judul }}
                            </button>
                        @endforeach
                    </div>
                    ...
                    <select name="buku_id" id="buku_id" class="transaksi-hidden-select"  required>
                        @foreach($buku as $item)
                            ...
                            <option value="{{ $item->id }}"
                                    data-stock="{{ $stokUntukEdit }}"
                                    @selected(old('buku_id', $transaksi->buku_id) == $item->id)>
                                {{ $item->judul }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="transaksi-form-row">
                ...
                <div class="transaksi-readonly">
                    <input type="text" id="stokTersedia" value="{{ ($transaksi->buku?->stok_tersedia ?? 0) + 1 }}" readonly>
                    <span>(readonly)</span>
                </div>
            </div>
            ...
            <div class="transaksi-form-row">
                ...
                <div class="transaksi-custom-date" data-date-picker>
                    ...
                    <input type="hidden"
                           ...
                           value="{{ old(
                                'tanggal_pinjam',
                                $transaksi->tanggal_pinjam?->format('Y-m-d')
                           ) }}"
                    >
            ...
            <div class="transaksi-form-row">
                ...
                <div class="transaksi-readonly">
                    <input type="text" id="jatuhTempoPreview" value="{{ $transaksi->tanggal_jatuh_tempo?->format('d-m-Y') ?? '00-00-0000' }}" readonly>
                    <span>(otomatis)</span>
                </div>
            </div>
            ...
            <div class="transaksi-form-row">
                ...
                <div class="transaksi-readonly">
                    <input type="text"
                           value="{{ $transaksi->status === 'terlambat'
                                ? 'Terlambat'
                                : 'Dipinjam' }}"
                           readonly
                    >
